...
class StaticResourceTest {
    ...
    @Test
    void indexHtmlShouldContainMarketplaceButton() throws Exception {
        ClassPathResource resource = new ClassPathResource("static/snap-agent/index.html");
        try (InputStream in = resource.getInputStream()) {
            String html = StreamUtils.copyToString(in, StandardCharsets.UTF_8);
            assertThat(html)
                    .as("index.html should contain marketplace nav button")
                    .contains("navMarketplaceBtn")
                    .contains("Skill 市场");
        }
    }

    @Test
    void appJsShouldContainMarketplaceModal() throws Exception {
        ClassPathResource resource = new ClassPathResource("static/snap-agent/app.js");
        try (InputStream in = resource.getInputStream()) {
            String js = StreamUtils.copyToString(in, StandardCharsets.UTF_8);
            assertThat(js)
                    .as("app.js should contain marketplace modal and install logic")
                    .contains("showMarketplaceModal")
                    .contains("installTemplate")
                    .contains("marketplace-grid")
                    .contains("/templates");
        }
    }

    @Test
    void styleCssShouldContainMarketplaceStyles() throws Exception {
        ClassPathResource resource = new ClassPathResource("static/snap-agent/style.css");
        try (InputStream in = resource.getInputStream()) {
            String css = StreamUtils.copyToString(in, StandardCharsets.UTF_8);
            assertThat(css)
                    .as("style.css should contain marketplace styles")
                    .contains(".marketplace-grid")
                    .contains(".marketplace-card")
                    .contains(".marketplace-install-btn");
        }
    }
}
