...
class SnapAgentAutoConfigurationTest {
    ...
    // ---- Knowledge (v2.x) ----
    // Old v0.7 KnowledgeBase/KnowledgeInjector/SimpleKeywordSearcher tests removed.
    // 2.x replaces with VectorStore/EmbeddingModel/RetrievalAugmentationAdvisor SPIs.
    // KnowledgeSedimentationService is wired when issue-closure.enabled=true.

    @Test
    void shouldCreateSedimentationServiceWhenIssueClosureEnabled() {
        contextRunner
                .withPropertyValues(
                        "snap-agent.enabled=true",
                        "snap-agent.llm.api-key=sk-test",
                        "snap-agent.issue-closure.enabled=true")
                .run(context -> {
                    assertThat(context).hasSingleBean(
                            cn.watsontech.snapagent.boot2x.knowledge.KnowledgeSedimentationService.class);
                });
    }

    @Test
    void shouldNotCreateSedimentationServiceWhenIssueClosureDisabled() {
        contextRunner
                .withPropertyValues(
                        "snap-agent.enabled=true",
                        "snap-agent.llm.api-key=sk-test",
                        "snap-agent.issue-closure.enabled=false")
                .run(context -> {
                    assertThat(context).doesNotHaveBean(
                            cn.watsontech.snapagent.boot2x.knowledge.KnowledgeSedimentationService.class);
                });
