...
| 锚点演示 | `snap-agent-anchor-demo` | 锚点问答功能演示应用 (Thymeleaf SKU 页面) | Spring Boot Starter Web + snap-agent-starter |
...
| `/anchor.js` | GET | 锚点问答客户端脚本（静态资源） |
| `/anchor/config` | GET | 锚点功能配置（公开） |
| `/anchor/preprocess` | POST | 锚点点击后预摘要 + 预分类 |
...
public class SnapAgentProperties {
    ...
    private Anchor anchor;      // 锚点问答 (enabled, disabled-paths, max-context-chars, preprocess)
...
| 锚点问答 | `snap-agent.anchor` | true (matchIfMissing) | `@ConditionalOnProperty` |
