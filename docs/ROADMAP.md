...
v0.9          IssueClosure + SolutionSuggester + VerificationRunner ← 已交付
    │                                                        问题问答闭环
    ▼
v1.0          ToolPlugin + Workflow + CostTracker            ← 已交付
                 工具插件生态 & 工作流 & 成本核算
```
...
| 组件 | 说明 |
|------|------|
| `ToolPlugin` (SPI) | 插件接口：pluginId + description + toolProviders() 列表 |
| `ToolPluginInfo` | 插件信息 DTO：pluginId, description, toolNames |
| `SimpleToolPluginRegistry` | 收集所有 ToolPlugin bean，提供 listPlugins() 元数据查询 |
| `snap-agent.tool-plugins.enabled` | 配置开关，默认 false |

#### 2. 工作流 (Workflow Orchestration)

| 组件 | 说明 |
|------|------|
| `Workflow` (SPI) | 工作流定义：id + description + steps |
| `WorkflowStep` | 步骤模型：name, skillId, inputs (含 ${ref} 引用), condition, onFailure 策略 |
| `WorkflowStepFailureStrategy` | 失败策略枚举：ABORT, SKIP, RETRY |
| `WorkflowExecutor` (SPI) | 执行器 SPI：execute(workflow, userId, model, triggerInputs) |
| `WorkflowResult` / `StepResult` | 结果模型：步骤结果 + 工作流状态 |
| `WorkflowStatus` | 状态枚举：RUNNING, COMPLETED, ABORTED, FAILED |
| `SimpleWorkflowExecutor` | 默认实现：顺序执行步骤，解析 ${ref} 引用，条件评估，失败策略 |
| `WorkflowRegistry` | 从 YAML 配置加载工作流定义 |
| `snap-agent.workflows.enabled` | 配置开关，默认 false |

#### 3. 成本核算 (Cost Accounting)

| 组件 | 说明 |
|------|------|
| `CostTracker` (SPI) | 成本追踪 SPI：record(), getSummary(), getRecords() |
| `CostRecord` | 单次 LLM 调用成本记录：userId, skillName, taskId, tokens, cost, timestamp |
| `CostSummary` | 聚合摘要：dimension, totalCost, requestCount, budget, utilization |
| `InMemoryCostTracker` | 默认实现：同步 List 存储，按 user/skill/global 维度聚合 |
| `CostCalculator` | Token→费用计算：inputPrice + outputPrice + cacheReadPrice |
| `snap-agent.cost.enabled` | 配置开关，默认 false |
| `snap-agent.cost.pricing` | Token 单价配置 (input/output/cacheRead per million) |
| `snap-agent.cost.budgets` | 预算限制 (perUserDaily/perSkillDaily/globalDaily) |
