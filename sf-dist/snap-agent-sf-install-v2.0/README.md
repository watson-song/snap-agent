# SnapAgent v2.0.0-SNAPSHOT — 集成指南（AI 可执行）
...
```
snap-agent-sf-install-v2.0.0-SNAPSHOT/
├── install-sf.sh                       # 一键安装脚本
├── lib/                                # Maven 本地文件仓库
│   └── cn/watsontech/snapagent/
│       ├── snap-agent-parent/2.0.0-SNAPSHOT/
│       │   └── snap-agent-parent-2.0.0-SNAPSHOT.pom   # ← 父 POM（含 dependencyManagement）
│       ├── snap-agent-core/2.0.0-SNAPSHOT/
│       │   ├── snap-agent-core-2.0.0-SNAPSHOT.jar
│       │   └── snap-agent-core-2.0.0-SNAPSHOT.pom
│       └── snap-agent-spring-boot-2x-starter/2.0.0-SNAPSHOT/
...
└── lib/                                    ← 新增
    └── cn/watsontech/snapagent/
        ├── snap-agent-parent/2.0.0-SNAPSHOT/
        │   └── snap-agent-parent-2.0.0-SNAPSHOT.pom   # ← 父 POM（不可少，子 pom 依赖它做版本管理）
        ├── snap-agent-core/2.0.0-SNAPSHOT/
        │   ├── snap-agent-core-2.0.0-SNAPSHOT.jar
        │   └── snap-agent-core-2.0.0-SNAPSHOT.pom
        └── snap-agent-spring-boot-2x-starter/2.0.0-SNAPSHOT/
...
> **内置 Skill 保护机制**：SnapAgent v2.0.0-SNAPSHOT.1+ 在 `ClasspathSkillScanner` 中实现了内置 Skill 保护——即使宿主项目的 `docs/skills/` 下有同名文件（如 `database-query.md`），SnapAgent JAR 内置版本始终优先，宿主版本会被自动跳过并记录 WARN 日志。上述 `<excludes>` 是额外的最佳实践，避免不必要的文件冲突。
...
> 1. 集成 SnapAgent 需要 LLM API 的认证凭据。提供两种接入方式：
>
>    **方式一（推荐）：领慧AI**
>    - 访问 https://ai.sf-express.com/project/enter/llm/model-square
>    - 进入「模型广场」→「APIKEY管理」，创建并获取 API Key
>    - 提供你的 API Key，我会填入 `auth-token` 配置
>    - 同时请告诉我你想用哪个模型（从模型广场选择，如 `glm-4-flash`、`qwen-plus` 等）
>
>    **方式二（备选）：ClaudeCode（cc-switch 网关）**
>    - 仅适用于本地开发环境，**顺丰云环境不可用**
>    - 提供你的工号，作为 `auth-token`
>    - 默认模型 `aliyun/glm-5.2`
>
>    **优先推荐使用领慧AI**，因为 ClaudeCode 在顺丰云环境无法访问。
>
...
snap-agent:
  ...
  llm:
    ...
    auth-token: <领慧API Key>           # ← 从领慧模型广场 → APIKEY管理 获取
    model: glm-4-flash                  # ← 从领慧模型广场选择
    allowed-models:
      - glm-4-flash
    # ---- 方式二（备选）：ClaudeCode（仅本地开发，顺丰云不可用）----
    # api-type: anthropic
...
> - **LLM 接入方式**：优先推荐领慧AI（`api-type: openai`），ClaudeCode（`api-type: anthropic`）仅用于本地开发，顺丰云环境不可用。
> - **领慧AI**：API Key 从 https://ai.sf-express.com/project/enter/llm/model-square → 模型广场 → APIKEY管理 获取。`base-url` 指向 `https://llm-model-hub-apis.sf-express.com`（OpenAI 兼容接口）。
> - **ClaudeCode**：`auth-token` 是使用者工号，作为 Bearer token 发送。**不要硬编码任何人的工号**，必须向用户确认后填入。
> - `upload-skills-dir: /app/deploy/skills` 是 SF 服务器上的 Skill 上传目录，重启后持久化。需确保该目录存在且应用账号有读写权限。
> - `allowed-models` 是服务端强制白名单，前端无法绕过。
> - `proxy-url` 是 HTTP 代理地址。当 K8s Pod 无法直连外网时，配置一个可达的 HTTP 代理（如 `http://10.x.x.x:3128`），LLM 请求将通过代理转发。留空则直连。
...
> | `/runs` | GET | 认证 | 查询当前用户任务列表（可按 status 过滤，如 `?status=RUNNING`） |
> | `/runs/{id}` | GET | 认证 | 查询任务状态 |
> | `/runs/{id}/transcript` | GET | 认证 | 获取任务完整 transcript |
> | `/runs/{id}/stream` | GET | 公开 | SSE 流（Controller 校验任务归属，支持断线重连自动 replay） |
...
>
> **context-path 场景**：如果后端配置了 `server.servlet.context-path`（如 `/api`），`proxy_pass` 需包含 context-path 前缀，否则后端收到 `/snap-agent/index.html` 会 404：
> ```nginx
> # 后端有 context-path=/api 时：
> location /snap-agent {
>     proxy_pass http://your-app-backend:1080/api/snap-agent;  # ← 注意 /api 前缀
>     # ... 其余配置同上
> }
> ```
> 浏览器 URL 保持 `/snap-agent/...`，Nginx 内部转发到后端 `/api/snap-agent/...`。
...
推送代码后检查 Jenkins/CI 构建日志：

1. **搜索 `install-file`**：如果使用方案二（maven-install-plugin），确认 3 个 snap-agent artifact（parent、core、starter）都安装成功，无 `Could not find artifact` 报错。
2. **搜索 `Could not find artifact`**：确认无依赖解析失败。如果出现，检查 `lib/` 目录是否完整提交到 Git（`git ls-files lib/ | head`）。
3. **搜索 `snap-agent-local`**：如果使用方案一（file:// 仓库），确认 Maven 从本地仓库解析，而非回退到远程仓库。如果只有 `Downloading from maven:` 没有 `snap-agent-local:`，说明 mirror 拦截了 file:// 仓库，需改用方案二。
4. **检查 fat JAR 内容**：构建成功后确认 jar 被正确打入：
   ```bash
   jar tf target/*.jar | grep snap-agent
   # 应包含 snap-agent-core 和 snap-agent-spring-boot-2x-starter 的 class 文件
   ```
...
| 可能原因 | 排查方法 | 修复 |
|---------|---------|------|
| 领慧 API 不可达 | `curl -v --connect-timeout 5 https://llm-model-hub-apis.sf-express.com/v1/models` | 联系运维开放 Pod 到该地址的出网规则，或配置 `proxy-url` 走代理 |
| ClaudeCode 代理不可达（仅本地开发） | `curl -v --connect-timeout 5 https://claudecode.sf-express.com/ccr` | ClaudeCode 在顺丰云环境不可用，仅限本地开发 |
| `timeout-seconds` 太小 | 检查配置 | 调大 `timeout-seconds`（默认 120s） |
| `auth-token` 错误或过期 | 确认领慧 API Key 有效（模型广场 → APIKEY管理） | 重新申请 API Key 并更新配置 |
...
| 可能原因 | 排查方法 | 修复 |
|---------|---------|------|
| CI/CD 的 `<mirror>` 拦截了 file:// 仓库 | 日志只有 `Downloading from maven:` 没有 `snap-agent-local:` | 改用 maven-install-plugin 方式（见 `pom-snippet.xml` 方案二） |
| `lib/` 缺少 parent POM | 检查 `lib/cn/watsontech/snapagent/snap-agent-parent/2.0.0-SNAPSHOT/` 是否存在 | 补充 `snap-agent-parent-2.0.0-SNAPSHOT.pom` 到 lib/ |
...
> **最常见原因**：CI/CD 的 `settings.xml` 配置了 `<mirror>`（如 `<mirrorOf>*</mirror>`），将所有仓库请求重定向到 Artifactory，`file://` 本地文件仓库被完全绕过。日志特征：只有 `Downloading from maven: https://artifactory...`，没有 `Downloading from snap-agent-local:`。此时 `file://` 仓库方案不可用，推荐改用 **maven-install-plugin:install-file** 方式（见 `pom-snippet.xml` 方案二），在 `validate` 阶段将 lib/ 中的 jar+pom 安装到本地 `.m2`，Maven 即可正常解析传递依赖。最后手段才用 system scope（方案三，deprecated）。
...
SnapAgent 自动保存每个 Skill 的会话历史（用户问题、Agent 回复、思考过程、工具调用、错误信息），重启后可恢复。

### 存储方式

- 默认实现：`FileConversationStore`，以 JSON 格式存储在 `{upload-skills-dir}/conversations/{userId}/{conversationId}.json`
- 自定义：宿主项目实现 `ConversationStore` 接口并声明为 Spring Bean（`@ConditionalOnMissingBean` 会覆盖默认实现），可替换为数据库存储等

### 功能

- **自动保存**：用户发送消息后立即保存，流式回复完成后保存完整 transcript（含思考、工具调用、错误）
- **断线重连**：页面刷新时自动检测运行中的任务（`GET /runs?status=RUNNING`），重新订阅 SSE 流，SSE 端点自动 replay 所有已产生的事件，完整重建 transcript
- **历史列表**：Web UI 底部 📜 按钮可查看当前 Skill 的历史会话列表
- **会话恢复**：点击历史列表中的会话可加载完整内容（含思考、工具调用等所有条目）
- **导出 Markdown**：历史列表中可下载某个会话为 Markdown 文件（含 YAML 元数据 + 按 role 分节）
- **删除会话**：历史列表中可删除某个会话

> **多轮对话**：每个 Skill 独立维护会话历史。切换 Skill 时自动保存当前会话并恢复目标 Skill 的上次会话。LLM 会收到历史对话上下文（仅 user/assistant 消息，不含思考/工具调用），支持多轮追问。
...
v2.0.0-SNAPSHOT 内置 4 个 Skill（打包在 JAR 的 classpath:/docs/skills/ 中，只读不可删除）：
...
| 项 | 值 |
|----|----|
| LLM 接入（推荐） | 领慧AI `https://llm-model-hub-apis.sf-express.com`（OpenAI 兼容） |
| LLM 接入（备选） | ClaudeCode `https://claudecode.sf-express.com/ccr`（仅本地开发，顺丰云不可用） |
| API Key | 领慧：模型广场 → APIKEY管理；ClaudeCode：使用者工号 |
| 默认模型 | `glm-4-flash`（领慧，可从模型广场选择其他） |
| 访问权限 | `snap-agent:access`（用户需拥有此权限标识） |
| Skill 上传目录 | `/app/deploy/skills` |
| 会话历史存储 | `{upload-skills-dir}/conversations/{userId}/`（JSON 文件） |
...
```bash
# 解压安装包
unzip snap-agent-sf-install-v2.0.0-SNAPSHOT.zip

# 运行安装脚本，指向你的 Spring Boot 项目
bash snap-agent-sf-install-v2.0.0-SNAPSHOT/install-sf.sh /path/to/your-project
```
...
SnapAgent 2.0.0-SNAPSHOT | Apache 2.0
